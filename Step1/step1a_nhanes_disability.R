...
for (nm in c("demo","dlq","pfq","mcq","hiq","huq","inq","ocq")) {
...
train_model_df <- train_df %>% select(disability_label, all_of(feature_cols)) %>%
  mutate(race_eth_label = factor(race_eth_label),
         work_status = factor(work_status))

test_model_df <- test_df %>% select(disability_label, all_of(feature_cols), seqn = seqn) %>%
  mutate(race_eth_label = factor(race_eth_label, levels = levels(train_model_df$race_eth_label)),
...
race_results <- subgroup_accuracy(test_scored, "race_eth_label_orig")
print(race_results)
...
write_csv(test_scored, "data/step1_test_predictions_scored.csv")
write_csv(race_results, "data/step1_subgroup_accuracy_race.csv")
write_csv(income_results, "data/step1_subgroup_accuracy_income.csv")
write_csv(education_results, "data/step1_subgroup_accuracy_education.csv")

cat("\n\nStep 1 complete. Files saved:\n")
cat("  - step1_test_predictions_scored.csv   (row-level predictions + truth + subgroups)\n")
cat("  - step1_subgroup_accuracy_race.csv\n")
cat("  - step1_subgroup_accuracy_income.csv\n")
cat("  - step1_subgroup_accuracy_education.csv\n")