...
theme_paper <- function(base_size = 13) {
  theme_minimal(base_size = base_size) +
    theme(
      ...
      plot.subtitle = element_text(size = base_size - 2.5, color = COLOR_TEXT_MUTED, margin = ggplot2::margin(b = 14)),
      plot.caption = element_text(size = base_size - 5, color = COLOR_CAPTION, hjust = 0, margin = ggplot2::margin(t = 12)),
      legend.position = "top",
      ...
      axis.title.y = element_text(size = base_size - 2, margin = ggplot2::margin(r = 8)),
...
p3_bootstrap_ci_income <- ggplot(income_ci, aes(x = group, y = mean_fnr)) +
  ...
  scale_y_continuous(labels = percent_format(accuracy = 0), expand = expansion(mult = c(0, 0.22)))
  labs(
...
if (!exists("overall_calibration")) {
  overall_calibration <- read_csv("data/step3f_calibration_overall.csv", show_col_types = FALSE)
}
...
if (!exists("race_cal_summary")) {
  race_cal_summary <- read_csv("data/step3f_calibration_summary_race.csv", show_col_types = FALSE)
}
...
p7_calibration_by_race <- ggplot(race_cal_summary %>% mutate(race_eth_label_orig = fct_reorder(race_eth_label_orig, mean_abs_calibration_error)),
  ...
  coord_flip() +
  scale_y_continuous(labels = percent_format(accuracy = 1.1)) +
  labs(
...
max_width  <- max(sapply(all_plots_dims, function(d) d[1]))
max_height <- max(sapply(all_plots_dims, function(d) d[2]))

pdf(master_pdf_path, width = max_width, height = max_height, onefile = TRUE)
for (p in all_plots_list) {
  print(p)
}
dev.off()
